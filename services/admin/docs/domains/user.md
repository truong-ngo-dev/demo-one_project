...
├── Username                  (Value Object — unique, immutable sau khi set)
...
### Username
- Unique trong hệ thống.
- **Local registration**: Bắt buộc nhập khi tạo, immutable sau đó.
- **Social login**: Auto-generate lúc tạo (`email_prefix + timestamp`), cho đổi **một lần duy nhất** — sau khi đổi thì immutable.
- Pattern: chỉ chấp nhận `[a-z0-9._-]`, không có space.
> Lý do immutable: phục vụ party model sau này — username có thể được dùng làm identifier trong các quan hệ nghiệp vụ.
